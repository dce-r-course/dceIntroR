...
Every time you use R, it operates from a specific *working directory*, *i.e.,* the folder where R looks for files to read and where it saves files you create.
...
However, the current working directory may not be the place where your data is stored. For example, your data might be stored in a folder called `data-raw`.

Then you must write the full path, such as:

```{r, eval=FALSE, message=FALSE, warning=FALSE}

data <- read.csv("C:/Users/YourName/Documents/my_project/data-raw/my_data.csv") 
```
...
setwd("C:/Users/YourName/Documents/my_project/data-raw/")
```
...
- `R` is the folder to save all your R code, whether it is an R Script, R Markdown, or Quarto file.
