...
You only need to install a package once on your computer. After it’s installed, `R` remembers it. That’s why it’s *not* best practice to include `install.packages()` inside your scripts. It’s something you do only when setting up your environment. However, it is recommended to update packages once in a while.
...
::: callout-note
Best practice would be to pass the name of the package as a string due to non-standard evaluation. However, in reality both `"tidyverse"` and `tidyverse` works. Although the following code becomes unclear:
```{r, eval=FALSE}
tidyverse <- 2+2
library(tidyverse)
```
