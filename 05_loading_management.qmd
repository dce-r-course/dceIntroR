...
However, you will need to know how to load different file formats.
...
data.csv <- data.table::fread("path_to_my_file/data.csv")
...
You can download the datasets used in this course [here](https://github.com/dce-r-course/dceIntroR/tree/main/data-raw).
...
We will get back to each function.
...
If you see `%>%`, you can change it to `|>` by entering `Shift + Crtl + P`, typing "pipe", and ticking "Use R's native **pipe** operator, \|\>"

Now we will go through some key functions mainly from the `dplyr` package.
...
- To prepare a subset of data for analysis or visualisation
...
- `contains("content")` - selects columns containing "content".
...
This can be particularly helpful when working with wide datasets.
...
This generates a column describing the `contact_type` variable. This doesn't make much sense to do though.. But we will use `mutate()` sensibly later on :)
...
`case_when()` also has a "default" option. You can use it like this: if this condition is true, do this; otherwise (default), do that. So, we could also have written:
...
2.  Look only at inpatient contacts.
3.  Define the first recorded hospitalisation as number 1
4.  Count the total number of hospitalisations per patient.
...
To do this, we can use the `group_by()` and `arrange()` functions.

*Note:* The `arrange()` per defaults order the variable in ascending order. To order in descending order, you can write `arrange(desc(x)).`
...
To fix this, we can use the `distinct()` function, which keeps unique rows within the defined groups.
...
The `by` argument is essential. For instance, we can combine data from `basic_data` and `diag_data` using `id.` You can join by several columns if relevant.

Example: Join `basic_data` and `diag_data`

Let's say you are interested in adding information on birth date and death date to the `diag_data`
