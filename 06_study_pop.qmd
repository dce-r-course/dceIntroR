...
To get some background information, take 2 minutes to read the `README` file in the `data-raw` folder.
...
basic_data <- readRDS(here::here("data-raw/basic_data.rds"))

diag_data <- readRDS(here::here("data-raw/diag_data.rds"))
...
- Which event?
  ...
  - We are interested in only first-time MIs. Accordingly, we will need to ensure that we order our data correctly before restricting to the first event.
...
Here, any changes to `data` will be saved in `data_updated`, which will appear in your Environment when you've run the code.

*Note*: you do not necessarily need to give the updated data a new name. You might also choose to overwrite a dataset.
...
We will be using the `str_detect` function from `stringr`. Essentially, what we should know about `str_detect` for now is that it takes the arguments: (1) which column; (2) what to search for.
...
So, the ICD-10 code for MI is `DI21`, which then would be "DI21". We are "lucky" that MI only has one ICD-10 code. If your pattern has multiple codes, you could write, for instance, "DA01\|DA02", saying "DA01" OR "DA02". And let's say you were interested in the codes DI801, DI802, and DI803, you could write "DI80\[1-3\]". Moreover, `str_detect` searches for any patterns that match your input pattern. So the input "DI21" would also keep "DI214" (subcodes).

But let's get back to filtering our data so only MI codes are kept.

```{r, warning=FALSE, message=FALSE}
study_pop_03 <- study_pop_02 |> 
  filter(
    str_detect(diag_a, "DI21") | str_detect(diag_b, "DI21")
  )
```

This code takes the `study_pop_02` dataset, filters rows so that only rows where `diag_a` or `diag_b` has a value that matches the pattern "DI21" are kept, and then saves the transformed data as `study_pop_03`.

*Note*: When the pattern is written as "DI21", str_detect will look for this pattern at any place in the character vector. So, a hypothetical pattern of "AIDI21" will remain. If you want to specify only to include character values starting with the pattern, you simply add "\^" at the beginning, *e.g.*, "\^DI21".

While this works, there are some simple ways we can optimise this code.
...
study_pop_03 <- study_pop_02 |> 
  filter(
    str_detect(diag_a, mi_code) | str_detect(diag_b, mi_code)
  )
```

This essentially does the same, but you only need to define the pattern once. This also means that if you need to revise your pattern, you only need to do it in this one place!
...
- `~ str_detect(.x, mi_code)`: This is an anonymous function where `.x` represents each column. `str_detect()` checks if the `mi_code` string is found in the column.
:::
...
study_pop_final <- 
  ...
  filter(str_detect(diag_a, mi_code) | str_detect(diag_b, mi_code)) |> 
...
```{r, warning = FALSE, message = FALSE}
study_pop_final <- study_pop_final |> 
  mutate(los = as.numeric(difftime(dis_date,index_date, units = "days")),
         .after = index_date)
