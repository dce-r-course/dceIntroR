...
#### Step 3. Identify a given covariate
...
```{r}
rm(data_baseline_mi, cov_hyperchol, cov_hypertension, hypertension_code, hyperchol_code)
```
...
-   Use for loop to apply the function for several comorbidities and combine the results into a single dataset

This method is scalable, readable, and easy to adapt to new comorbidities.

#### Step 1. Make a baseline dataset

Make a baseline set onto which we will add covariate indicators
...
Let's make a function from the code we used in the previous step 3.

We first `print` the name of the covariate. This help us keeping track on the progress.

We then set the code pattern to be used. We simply extract the code related to the covariate as defined in our comorbidity_definions vector.

We then identify whether a given contact has a record with the disease of interest.

We then only keep relevant contacts and keep id and the covariate indicator.

Finally, we return the specific dataset containing the covariate indicator.
...
#### Step 4. Apply the function using a for loop

Essentially, the for loop runs the function above for each disease name in our comorbidity_definition file.
It then adds the covariate indicator to the baseline ´data_covariate_mi´. 
...
Combine information on age and sex with the covariate data to get a full baseline dataset.

Then, we replace NA with 0 for the covariate dummy variables. We could write out each covariate name, but we might as well be lazy and get some help from the comorbidity_definions :) 
