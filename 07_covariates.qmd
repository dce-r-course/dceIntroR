...
In the `basic_data`, we can obtain information on age and sex from the variables `birth_date` and `sex`.
...
data_cov_02 <- data_cov_01 |> 
  mutate(
    age = floor(as.numeric(difftime(index_date,birth_date, units = "days"))/365.25)
  )
...
- `theme_bw()`
  ...
  Applies a minimal black-and-white theme — removes background shading and grid clutter for a cleaner appearance. There are tons of themes you can use. Moreover, using the `theme()` function, you can customise your layout extensively.
:::
...
2.  Keep only diagnoses recorded before the index date and within the lookback period
...
data_cov_diag <- data_baseline_mi |> 
  ...
  mutate(
    diff = as.numeric(difftime(adm_date,index_date, units = "days")) # can also be switched, then adjust definition of lookback
  ) |> 
  filter(
    diff < 0,
    diff > (-365.25*lookback_years)
  ) 

data_cov_diag
```

Now, we could do some checks. This is easily done with some base R manouvers.

```{r}
# Is the largest difference between index date and covariate admission lower than or equal to the lookback period?
min(data_cov_diag[["diff"]])

# Is the lowest difference between index date and covariate admission lower than 0 days?
max(data_cov_diag[["diff"]])
...
Note: This approach keeps only those with a given covariate within the lookback period.
...
cov_hypertension <- data_cov_diag |> 
  mutate(
    hypertension = 
      case_when(
        str_detect(diag_a, hypertension_code) | str_detect(diag_b, hypertension_code) ~ 1,
        .default = 0
      )) |> 
...
We therefore need to add the covariate to the baseline and replace `NA` with `0.`
...
cov_hyperchol <- data_cov_diag |> 
  mutate(
    hyperchol = 
      case_when(
        str_detect(diag_a, hyperchol_code) | str_detect(diag_b, hyperchol_code) ~ 1,
        .default = 0
      )) |> 
  filter(hyperchol == 1) |> 
  select(id, hyperchol) |> 
...
While this approach does work, it gets slightly redundant, and we are repeating ourselves a lot!
...
- Write a **function** that identifies patients who have a history of a given comorbidity
...
1.  We first `message` the name of the covariate. This help us keeping track on the progress.
...
identify_comorbidity <- function(data, disease_name) {
  ...
  disease_name_sym <- rlang::sym(disease_name)
  ...
  cov_data <- data |> 
    ...
    # step 03
    dplyr::mutate(
      !!disease_name_sym := dplyr::case_when(
        str_detect(diag_a, code) | str_detect(diag_b, code) ~ 1,
        .default = 0
      )
    ) |> 
    
    # step 04
    dplyr::filter(!!disease_name_sym == 1) |> 
    dplyr::select(id, !!disease_name_sym) |> 
    dplyr::distinct()
