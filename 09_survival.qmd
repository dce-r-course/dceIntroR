...
Now, we will setup our dataset for a time-to-event analysis. We will follow patients from `index_date` until death, 1 year of follow-up, or December 31, 2024, whichever occurred first.

*Note:* We here just assume no emigration.
...
data_tte <- data_tte |> 
  dplyr::mutate(
    max_fu = lubridate::add_with_rollback(lubridate::ymd(index_date), lubridate::years(1)), # 1 year of follow-up
...
```{r, warning=FALSE, message=FALSE}
data_tte <- data_tte |> 
  mutate(across(c(death_date, max_fu, stop_study), ~ lubridate::ymd(.x)), # ensure that dates are <date> format
...
data_tte <- data_tte |> 
  mutate(
         time = as.numeric(difftime(end_date, index_date, units = "days")) # here in days
         )
...
#### Explore estimates at specific time points

Lets generate a dataset with mortality risk estimates with 1 day intervals from day 0 until end of follow-up.
...
We can also plot a mortality risk curve using the same dataset.
